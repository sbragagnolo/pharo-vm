...
public class StackView extends View {
	...
	protected void onLayout(boolean changed, int left, int top, int right,
			int bottom) {
		Log.w(TAG, "onLayout" + changed + "  " + left + "  " + top + "  "
				+ right + "  " + bottom);
		if (!changed)
			return;
	...
	protected void onDraw(Canvas canvas) {
		...
		if (canvas.getClipBounds(dirtyRect)) {
			vm.updateDisplay(bits, width, height, depth, dirtyRect.left,
					dirtyRect.top, dirtyRect.right, dirtyRect.bottom);
		}

		super.onDraw(canvas);
		canvas.drawColor(-1);
		Bitmap theRenderingWorld = Bitmap.createBitmap(bits, width, height,
				Bitmap.Config.valueOf("ARGB_8888"));
		canvas.drawBitmap(theRenderingWorld, 0, 0, paint);
		if (vm.updatingViewProcessIsActive())
			this.postDelayed(new StackReativate(), 100);

	}

	final class StackReativate implements Runnable {
		public void run() {
			Log.w(TAG, "StackReativate");
